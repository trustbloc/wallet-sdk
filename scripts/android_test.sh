...
adb reverse tcp:8075 tcp:8075 && adb reverse tcp:8072 tcp:8072 &&  adb reverse tcp:9229 tcp:9229 && \
  ANDROID_EMULATOR_WAIT_TIME_BEFORE_KILL=120 $GRADLE_CMD app:connectedAndroidTest -PINITIATE_ISSUANCE_URL="${INITIATE_ISSUANCE_URL}" -PINITIATE_VERIFICATION_URL="${INITIATE_VERIFICATION_URL}" -PINITIATE_ISSUANCE_URLS_AUTH_CODE_FLOW="${INITIATE_ISSUANCE_URLS_AUTH_CODE_FLOW}"
